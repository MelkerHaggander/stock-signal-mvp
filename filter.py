...
def _mentions_company(headline: str, symbol: str, name: str) -> bool:
...
def filter_noise(data: NormalizedData) -> NormalizedData:
    ...
    for item in data.news:
        ...
        if data.asset.symbol or data.asset.name:
            if not _mentions_company(item.headline, data.asset.symbol, data.asset.name):
                continue
